...
public abstract class DefaultCoreCardGridPart<S extends DefaultCoreCardGridPart<S, O>, O extends CoreCardGridPartOptions> implements ComponentPart , CoreCardGrid {
    @Override
    ...
    @Override
    public CoreCardGridPart setGap(Integer gap) {
        setColumnGap(gap);
        return setRowGap(gap);
    }

    @Override
    public CoreCardGridPart setGap(Integer gap, State state) {
        setColumnGap(gap, state);
        return setRowGap(gap, state);
    }

    @Override
    public Integer getGap(State state) {
        return getColumnGap(state);
    }
    ...
    @Override
    public CoreCardGridPart setPositionMode(CardGridPositionMode positionMode) {
        return self();
    }
    ...
    protected CoreContainer cardContainer;

    protected Map<State, Integer> columnGap = new HashMap<>();

    protected Map<State, Integer> rowGap = new HashMap<>();

    protected Map<State, Integer> cardWidth = new HashMap<>();

    protected Map<State, Integer> cardHeight = new HashMap<>();

    protected Boolean expandableCards;

    protected DefaultCoreCardGridPart(O options) {
    }
    ...
    @Override
    public CoreCardGrid getComponent() {
        return ((CoreCardGrid) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        if (needsScrim(options)) {
            scrim = createScrim(options);
        }
        cardContainer = createCardContainer(options);
        setupReferences();
        return true;
    }
    ...
    public CoreContainer getCardContainer() {
    ...
    private CoreContainer createCardContainer(O options) {
        if ((cardContainer != null) || (options == null))
            return cardContainer;
        ...
        cardContainerOptions.setParent(this);
        ComponentCreator cardContainerCreator = options.getCardContainerCreator();
        cardContainer = (cardContainerCreator != null) ? ((CoreContainer) (cardContainerCreator.create(cardContainerOptions))) : getComponentFactory().createContainer(cardContainerOptions);
        getParentComponentPart().add(cardContainer);
        return cardContainer;
    }

    @Override
    public S add(CoreComponent child) {
        cardContainer.add(child);
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreCardGridPart component = ((CoreCardGridPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getColumnGapOption(), component::setColumnGap);
        applicator.add(options.getRowGapOption(), component::setRowGap);
        applicator.add(options.getGapOption(), component::setGap);
        applicator.add(options.getCardWidthOption(), component::setCardWidth);
        applicator.add(options.getCardHeightOption(), component::setCardHeight);
        applicator.add(options.getPositionModeOption(), component::setPositionMode);
        applicator.add(options.getAnimationEnabledOption(), component::setAnimationEnabled);
        applicator.add(options.getExpandableCardsOption(), component::setExpandableCards);
        if (states != null)
            states.stream().forEach((State state) -> {
                // Apply options for each (non-default) state
                applicator.add(options.getColumnGapOption(state), state, component::setColumnGap);
                applicator.add(options.getRowGapOption(state), state, component::setRowGap);
                applicator.add(options.getGapOption(state), state, component::setGap);
                applicator.add(options.getCardWidthOption(state), state, component::setCardWidth);
                applicator.add(options.getCardHeightOption(state), state, component::setCardHeight);
            });
    ...
    public Integer getColumnGap() {
        return columnGap.get(null);
    }

    @Override
    public Integer getColumnGap(State state) {
        return columnGap.get(state);
    }

    @Override
    public S setColumnGap(Integer columnGap) {
        this.columnGap.put(null, columnGap);
        return self();
    }
    ...
    public S setColumnGap(Integer columnGap, State state) {
        this.columnGap.put(state, columnGap);
        return self();
    }

    @Override
    public Integer getRowGap() {
        return rowGap.get(null);
    }

    @Override
    public Integer getRowGap(State state) {
        return rowGap.get(state);
    }

    @Override
    public S setRowGap(Integer rowGap) {
        this.rowGap.put(null, rowGap);
        return self();
    }
    ...
    public Integer getCardWidth() {
        return cardWidth.get(null);
    }

    @Override
    public Integer getCardWidth(State state) {
        return cardWidth.get(state);
    }

    @Override
    public S setCardWidth(Integer cardWidth) {
        this.cardWidth.put(null, cardWidth);
        return self();
    }
    ...
    public S setCardWidth(Integer cardWidth, State state) {
        this.cardWidth.put(state, cardWidth);
        return self();
    }

    @Override
    public Integer getCardHeight() {
        return cardHeight.get(null);
    }

    @Override
    public Integer getCardHeight(State state) {
        return cardHeight.get(state);
    }

    @Override
    public S setCardHeight(Integer cardHeight) {
        this.cardHeight.put(null, cardHeight);
        return self();
    }
    ...
    public S setCardHeight(Integer cardHeight, State state) {
        this.cardHeight.put(state, cardHeight);
        return self();
    }

    @Override
    public boolean isExpandableCards() {
        return Boolean.TRUE.equals(expandableCards);
    }

    @Override
    public S setExpandableCards(Boolean expandableCards) {
        this.expandableCards = expandableCards;
        return self();
    }
