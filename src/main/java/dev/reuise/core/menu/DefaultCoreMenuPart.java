...
public abstract class DefaultCoreMenuPart<S extends DefaultCoreMenuPart<S, O>, O extends CoreMenuPartOptions> implements ComponentPart , CoreMenu {
    ...
    protected CoreListView list;

    protected DefaultCoreMenuPart(O options) {
    }
    ...
    @Override
    public CoreMenu getComponent() {
        return ((CoreMenu) (baseComponentPart.getComponent()));
    }
    ...
    private CoreListView createList(O options) {
        if ((list != null) || (options == null))
            return list;

        CoreListViewOptions listOptions = options.getListOptions();
        if (listOptions == null)
            return list;

        listOptions.setParent(this);
        ComponentCreator listCreator = options.getListCreator();
        list = (listCreator != null) ? ((CoreListView) (listCreator.create(listOptions))) : getComponentFactory().createListView(listOptions);
        getParentComponentPart().add(list);
        return list;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreMenuPart component = ((CoreMenuPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getAnchorOption(), component::setAnchor);
        applicator.add(options.getItemsOption(), component::setItems);
        applicator.add(options.getAnchorPositionOption(), component::setAnchorPosition);
        applicator.add(options.getMenuPositionOption(), component::setMenuPosition);
    }
