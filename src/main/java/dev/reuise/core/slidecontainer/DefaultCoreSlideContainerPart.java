...
public abstract class DefaultCoreSlideContainerPart<S extends DefaultCoreSlideContainerPart<S, O>, O extends CoreSlideContainerPartOptions> implements ComponentPart , CoreSlideContainer {
    ...
    protected CoreContainer slideAreaBefore;

    protected CoreContainer slideArea;

    protected CoreContainer slideAreaAfter;

    protected SlideContainerSlideButtonOptions slideButtonOptions;

    protected DefaultCoreSlideContainerPart(O options) {
    }
    ...
    @Override
    public CoreSlideContainer getComponent() {
        return ((CoreSlideContainer) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        slideAreaBefore = createSlideAreaBefore(options);
        slideArea = createSlideArea(options);
        slideAreaAfter = createSlideAreaAfter(options);
        setupReferences();
        return true;
    }

    public CoreContainer getSlideAreaBefore() {
        return slideAreaBefore;
    }

    private boolean needsSlideAreaBefore(O options) {
        if (slideAreaBefore != null)
            return true;
    ...
    private CoreContainer createSlideAreaBefore(O options) {
        if ((slideAreaBefore != null) || (options == null))
            return slideAreaBefore;

        CoreContainerOptions slideAreaBeforeOptions = options.getSlideAreaBeforeOptions();
        if (slideAreaBeforeOptions == null)
            return slideAreaBefore;

        slideAreaBeforeOptions.setParent(this);
        ComponentCreator slideAreaBeforeCreator = options.getSlideAreaBeforeCreator();
        slideAreaBefore = (slideAreaBeforeCreator != null) ? ((CoreContainer) (slideAreaBeforeCreator.create(slideAreaBeforeOptions))) : getComponentFactory().createContainer(slideAreaBeforeOptions);
        getParentComponentPart().add(slideAreaBefore);
        return slideAreaBefore;
    }

    public CoreContainer getSlideArea() {
        return slideArea;
    }

    private boolean needsSlideArea(O options) {
        if (slideArea != null)
            return true;
    ...
    private CoreContainer createSlideArea(O options) {
        if ((slideArea != null) || (options == null))
            return slideArea;

        CoreContainerOptions slideAreaOptions = options.getSlideAreaOptions();
        if (slideAreaOptions == null)
            return slideArea;

        slideAreaOptions.setParent(this);
        ComponentCreator slideAreaCreator = options.getSlideAreaCreator();
        slideArea = (slideAreaCreator != null) ? ((CoreContainer) (slideAreaCreator.create(slideAreaOptions))) : getComponentFactory().createContainer(slideAreaOptions);
        getParentComponentPart().add(slideArea);
        return slideArea;
    }
    ...
    public void insertBefore(CoreComponent child, CoreComponent beforeChild) {
        slideArea.insertBefore(child, beforeChild);
    }

    @Override
    public S addText(String text) {
        slideArea.addText(text);
        return self();
    }

    @Override
    public void remove(CoreComponent child) {
        slideArea.remove(child);
    }
    ...
    public List<CoreComponent> getChildren() {
        return slideArea.getChildren();
    }

    public CoreContainer getSlideAreaAfter() {
        return slideAreaAfter;
    }

    private boolean needsSlideAreaAfter(O options) {
        if (slideAreaAfter != null)
            return true;
    ...
    private CoreContainer createSlideAreaAfter(O options) {
        if ((slideAreaAfter != null) || (options == null))
            return slideAreaAfter;

        CoreContainerOptions slideAreaAfterOptions = options.getSlideAreaAfterOptions();
        if (slideAreaAfterOptions == null)
            return slideAreaAfter;

        slideAreaAfterOptions.setParent(this);
        ComponentCreator slideAreaAfterCreator = options.getSlideAreaAfterCreator();
        slideAreaAfter = (slideAreaAfterCreator != null) ? ((CoreContainer) (slideAreaAfterCreator.create(slideAreaAfterOptions))) : getComponentFactory().createContainer(slideAreaAfterOptions);
        getParentComponentPart().add(slideAreaAfter);
        return slideAreaAfter;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreSlideContainerPart component = ((CoreSlideContainerPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getSlideDirectionOption(), component::setSlideDirection);
        applicator.add(options.getShowSlideBarsOption(), component::setShowSlideBars);
        applicator.add(options.getSlideButtonOptionsOption(), component::setSlideButtonOptions);
    }
