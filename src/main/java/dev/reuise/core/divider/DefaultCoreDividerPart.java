...
public abstract class DefaultCoreDividerPart<S extends DefaultCoreDividerPart<S, O>, O extends CoreDividerPartOptions> implements ComponentPart , CoreDivider {
    @Override
    ...
    @Override
    public S setDirection(DividerDirection dividerDirection) {
        return self();
    }
    ...
    protected CoreContainer startLine;

    protected CoreInlineText labelContainer;

    protected CoreContainer endLine;

    protected DefaultCoreDividerPart(O options) {
    }
    ...
    @Override
    public CoreDivider getComponent() {
        return ((CoreDivider) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        startLine = createStartLine(options);
        labelContainer = createLabelContainer(options);
        endLine = createEndLine(options);
        setupReferences();
        return true;
    }

    public CoreContainer getStartLine() {
        return startLine;
    }
    ...
    private CoreContainer createStartLine(O options) {
        if ((startLine != null) || (options == null))
            return startLine;

        CoreContainerOptions startLineOptions = options.getStartLineOptions();
        if (startLineOptions == null)
            return startLine;

        startLineOptions.setParent(this);
        ComponentCreator startLineCreator = options.getStartLineCreator();
        startLine = (startLineCreator != null) ? ((CoreContainer) (startLineCreator.create(startLineOptions))) : getComponentFactory().createContainer(startLineOptions);
        getParentComponentPart().add(startLine);
        return startLine;
    }
    ...
    private CoreInlineText createLabelContainer(O options) {
        ...
        getParentComponentPart().add(labelContainer);
        return labelContainer;
    }

    public CoreContainer getEndLine() {
        return endLine;
    }
    ...
    private CoreContainer createEndLine(O options) {
        if ((endLine != null) || (options == null))
            return endLine;

        CoreContainerOptions endLineOptions = options.getEndLineOptions();
        if (endLineOptions == null)
            return endLine;

        endLineOptions.setParent(this);
        ComponentCreator endLineCreator = options.getEndLineCreator();
        endLine = (endLineCreator != null) ? ((CoreContainer) (endLineCreator.create(endLineOptions))) : getComponentFactory().createContainer(endLineOptions);
        getParentComponentPart().add(endLine);
        return endLine;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreDividerPart component = ((CoreDividerPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getLabelOption(), component::setLabel);
        applicator.add(options.getLabelAlignmentOption(), component::setLabelAlignment);
        applicator.add(options.getThicknessOption(), component::setThickness);
    ...
    public S setLabel(String label) {
        CoreInlineText labelContainer = getLabelContainer();
        if ((label != null) && (labelContainer != null))
            getLabelContainer().setText(label);
