...
public abstract class DefaultCoreCheckboxPart<S extends DefaultCoreCheckboxPart<S, O>, O extends CoreCheckboxPartOptions> implements ComponentPart , CoreCheckbox {
    private CoreParentComponentPart parentComponentPart;

    private CoreBaseComponentPart baseComponentPart;

    protected CoreBasicCheckbox checkbox;

    protected CoreLabel labelContainer;

    protected DefaultCoreCheckboxPart(O options) {
    }
    ...
    @Override
    public CoreCheckbox getComponent() {
        return ((CoreCheckbox) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        checkbox = createCheckbox(options);
        if (needsLabelContainer(options)) {
    ...
    public CoreBasicCheckbox getCheckbox() {
        return checkbox;
    }
    ...
    private CoreBasicCheckbox createCheckbox(O options) {
        if ((checkbox != null) || (options == null))
            return checkbox;

        CoreBasicCheckboxOptions checkboxOptions = options.getCheckboxOptions();
        if (checkboxOptions == null)
            return checkbox;

        checkboxOptions.setParent(this);
        ComponentCreator checkboxCreator = options.getCheckboxCreator();
        checkbox = (checkboxCreator != null) ? ((CoreBasicCheckbox) (checkboxCreator.create(checkboxOptions))) : getComponentFactory().createBasicCheckbox(checkboxOptions);
        getParentComponentPart().add(checkbox);
        return checkbox;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreCheckboxPart component = ((CoreCheckboxPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getLabelOption(), component::setLabel);
        applicator.add(options.getCheckedOption(), component::setChecked);
    }
