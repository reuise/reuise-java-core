...
public abstract class DefaultCoreAppLayoutPart<S extends DefaultCoreAppLayoutPart<S, O>, O extends CoreAppLayoutPartOptions> implements ComponentPart , CoreAppLayout {
    private CoreParentComponentPart parentComponentPart;

    private CoreBaseComponentPart baseComponentPart;

    protected CoreContainer drawerScrim;

    protected CoreSplitContainer drawerSplit;

    protected CoreSplitContainer topAppBarSplit;

    protected CoreAppLayoutBody body;

    protected CoreTopAppBar topAppBar;

    protected CoreBottomAppBar bottomAppBar;
    ...
    @Override
    public CoreAppLayout getComponent() {
        return ((CoreAppLayout) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        drawerScrim = createDrawerScrim(options);
        if (needsDrawerSplit(options)) {
            drawerSplit = createDrawerSplit(options);
        }
        if (needsTopAppBarSplit(options)) {
            topAppBarSplit = createTopAppBarSplit(options);
        }
    ...
    public CoreContainer getDrawerScrim() {
        return drawerScrim;
    }
    ...
    private CoreContainer createDrawerScrim(O options) {
        if ((drawerScrim != null) || (options == null))
            return drawerScrim;

        CoreContainerOptions drawerScrimOptions = options.getDrawerScrimOptions();
        if (drawerScrimOptions == null)
            return drawerScrim;

        drawerScrimOptions.setParent(this);
        ComponentCreator drawerScrimCreator = options.getDrawerScrimCreator();
        drawerScrim = (drawerScrimCreator != null) ? ((CoreContainer) (drawerScrimCreator.create(drawerScrimOptions))) : getComponentFactory().createContainer(drawerScrimOptions);
        getParentComponentPart().add(drawerScrim);
        return drawerScrim;
    }

    public CoreSplitContainer getDrawerSplit() {
        return drawerSplit;
    }
    ...
    private boolean needsDrawerSplit(O options) {
        ...
        if (options.getDrawer() != null)
            return true;

        return false;
    }

    private CoreSplitContainer createDrawerSplit(O options) {
        if ((drawerSplit != null) || (options == null))
            return drawerSplit;

        CoreSplitContainerOptions drawerSplitOptions = options.getDrawerSplitOptions();
        if (drawerSplitOptions == null)
            return drawerSplit;

        CoreParentComponentPart parent = (topAppBarSplit != null) ? topAppBarSplit : getParentComponentPart();
        drawerSplitOptions.setParent(topAppBarSplit != null ? topAppBarSplit : this);
        ComponentCreator drawerSplitCreator = options.getDrawerSplitCreator();
        drawerSplit = (drawerSplitCreator != null) ? ((CoreSplitContainer) (drawerSplitCreator.create(drawerSplitOptions))) : getComponentFactory().createSplitContainer(drawerSplitOptions);
        parent.add(drawerSplit);
        return drawerSplit;
    }

    public CoreSplitContainer getTopAppBarSplit() {
        return topAppBarSplit;
    }
    ...
    private boolean needsTopAppBarSplit(O options) {
        ...
        if (options.getTopAppBar() != null)
            return true;

        return false;
    }

    private CoreSplitContainer createTopAppBarSplit(O options) {
        if ((topAppBarSplit != null) || (options == null))
            return topAppBarSplit;
        ...
        CoreParentComponentPart parent = (drawerSplit != null) ? drawerSplit : getParentComponentPart();
        topAppBarSplitOptions.setParent(drawerSplit != null ? drawerSplit : this);
        ComponentCreator topAppBarSplitCreator = options.getTopAppBarSplitCreator();
        topAppBarSplit = (topAppBarSplitCreator != null) ? ((CoreSplitContainer) (topAppBarSplitCreator.create(topAppBarSplitOptions))) : getComponentFactory().createSplitContainer(topAppBarSplitOptions);
        parent.add(topAppBarSplit);
        return topAppBarSplit;
    }

    public CoreAppLayoutBody getBody() {
        return body;
    }
    ...
    private CoreAppLayoutBody createBody(O options) {
        if ((body != null) || (options == null))
            return body;

        CoreAppLayoutBodyOptions bodyOptions = options.getBodyOptions();
        if (bodyOptions == null)
            return body;

        bodyOptions.setParent(this);
        ComponentCreator bodyCreator = options.getBodyCreator();
        body = (bodyCreator != null) ? ((CoreAppLayoutBody) (bodyCreator.create(bodyOptions))) : getComponentFactory().createAppLayoutBody(bodyOptions);
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        ...
        applicator.add(options.getTopAppBarOption(), component::setTopAppBar);
        applicator.add(options.getBottomAppBarOption(), component::setBottomAppBar);
        applicator.add(options.getDrawerOption(), component::setDrawer);
    }
