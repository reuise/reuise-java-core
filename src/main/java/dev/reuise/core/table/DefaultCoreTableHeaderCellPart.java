...
public abstract class DefaultCoreTableHeaderCellPart<S extends DefaultCoreTableHeaderCellPart<S, O>, O extends CoreTableHeaderCellPartOptions> implements ComponentPart , CoreTableHeaderCell {
    ...
    protected CoreMenu menu;

    protected CoreIconButton menuButton;

    protected Boolean hasMenuButton;

    protected DefaultCoreTableHeaderCellPart(O options) {
    }
    ...
    @Override
    public CoreTableHeaderCell getComponent() {
        return ((CoreTableHeaderCell) (baseComponentPart.getComponent()));
    }
    ...
    private boolean needsMenu(O options) {
        ...
        return options.hasMenuOptions();
    }

    private CoreMenu createMenu(O options) {
        if ((menu != null) || (options == null))
            return menu;

        CoreMenuOptions menuOptions = options.getMenuOptions();
        if (menuOptions == null)
            return menu;

        menuOptions.setParent(this);
        ComponentCreator menuCreator = options.getMenuCreator();
        menu = (menuCreator != null) ? ((CoreMenu) (menuCreator.create(menuOptions))) : getComponentFactory().createMenu(menuOptions);
        getParentComponentPart().add(menu);
        return menu;
    }

    public CoreIconButton getMenuButton() {
        return menuButton;
    }
    ...
    private boolean needsMenuButton(O options) {
        ...
        if ((menu != null) || options.isHasMenuButton())
            return true;

        return false;
    }

    private CoreIconButton createMenuButton(O options) {
        if ((menuButton != null) || (options == null))
            return menuButton;

        CoreIconButtonOptions menuButtonOptions = options.getMenuButtonOptions();
        if (menuButtonOptions == null)
            return menuButton;

        menuButtonOptions.setParent(this);
        ComponentCreator menuButtonCreator = options.getMenuButtonCreator();
        menuButton = (menuButtonCreator != null) ? ((CoreIconButton) (menuButtonCreator.create(menuButtonOptions))) : getComponentFactory().createIconButton(menuButtonOptions);
        getParentComponentPart().add(menuButton);
        return menuButton;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreTableHeaderCellPart component = ((CoreTableHeaderCellPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getLabelOption(), component::setLabel);
        applicator.add(options.getHasMenuButtonOption(), component::setHasMenuButton);
    }
    ...
    public boolean isHasMenuButton() {
        return Boolean.TRUE.equals(hasMenuButton);
    }

    @Override
    public S setHasMenuButton(Boolean hasMenuButton) {
        this.hasMenuButton = hasMenuButton;
        return self();
    }
}