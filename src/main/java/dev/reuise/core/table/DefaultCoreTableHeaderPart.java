...
public abstract class DefaultCoreTableHeaderPart<S extends DefaultCoreTableHeaderPart<S, O>, O extends CoreTableHeaderPartOptions> implements ComponentPart , CoreTableHeader {
    @Override
    ...
    protected CoreTableHeaderRow row;

    protected DefaultCoreTableHeaderPart(O options) {
    }
    ...
    @Override
    public CoreTableHeader getComponent() {
        return ((CoreTableHeader) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        row = createRow(options);
        setupReferences();
        return true;
    }

    public CoreTableHeaderRow getRow() {
        return row;
    }
    ...
    private CoreTableHeaderRow createRow(O options) {
        if ((row != null) || (options == null))
            return row;

        CoreTableHeaderRowOptions rowOptions = options.getRowOptions();
        if (rowOptions == null)
            return row;

        rowOptions.setParent(this);
        ComponentCreator rowCreator = options.getRowCreator();
        row = (rowCreator != null) ? ((CoreTableHeaderRow) (rowCreator.create(rowOptions))) : getComponentFactory().createTableHeaderRow(rowOptions);
        getParentComponentPart().add(row);
        return row;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreTableHeaderPart component = ((CoreTableHeaderPart) (options.getComponent()));
        if (component == null)
