...
public abstract class DefaultCoreProgressIndicatorPart<S extends DefaultCoreProgressIndicatorPart<S, O>, O extends CoreProgressIndicatorPartOptions> implements CoreProgressIndicator , ComponentPart {
    private CoreParentComponentPart parentComponentPart;

    private CoreBaseComponentPart baseComponentPart;

    protected CoreContainer track;

    protected ProgressIndicatorTiming timing;

    protected Integer progress;

    protected ProgressIndicatorVisibilityBehaviour visibilityBehaviour;

    protected DefaultCoreProgressIndicatorPart(O options) {
    }
    ...
    @Override
    public CoreProgressIndicator getComponent() {
        return ((CoreProgressIndicator) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        track = createTrack(options);
        setupReferences();
        return true;
    }

    public CoreContainer getTrack() {
        return track;
    }
    ...
    private CoreContainer createTrack(O options) {
        if ((track != null) || (options == null))
            return track;

        CoreContainerOptions trackOptions = options.getTrackOptions();
        if (trackOptions == null)
            return track;

        trackOptions.setParent(this);
        ComponentCreator trackCreator = options.getTrackCreator();
        track = (trackCreator != null) ? ((CoreContainer) (trackCreator.create(trackOptions))) : getComponentFactory().createContainer(trackOptions);
        getParentComponentPart().add(track);
        return track;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreProgressIndicatorPart component = ((CoreProgressIndicatorPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getLayoutOption(), component::setLayout);
        applicator.add(options.getTimingOption(), component::setTiming);
        applicator.add(options.getProgressOption(), component::setProgress);
        applicator.add(options.getStartedOption(), component::setStarted);
        applicator.add(options.getVisibilityBehaviourOption(), component::setVisibilityBehaviour);
    }
