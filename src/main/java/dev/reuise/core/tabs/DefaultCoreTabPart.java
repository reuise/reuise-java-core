...
public abstract class DefaultCoreTabPart<S extends DefaultCoreTabPart<S, O>, O extends CoreTabPartOptions> implements ComponentPart , CoreTab {
    ...
    protected CoreBadge badge;

    protected CoreIcon icon;

    protected CoreInlineText labelContainer;

    protected DefaultCoreTabPart(O options) {
    }
    ...
    @Override
    public CoreTab getComponent() {
        return ((CoreTab) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        if (needsLink(options)) {
            link = createLink(options);
        }
        badge = createBadge(options);
        icon = createIcon(options);
        labelContainer = createLabelContainer(options);
        setupReferences();
        return true;
    }

    public CoreLink getLink() {
        return link;
    }
    ...
    private CoreLink createLink(O options) {
        if ((link != null) || (options == null))
            return link;

        CoreLinkOptions linkOptions = options.getOrCreateLinkOptions();
        if (linkOptions == null)
            return link;

        linkOptions.setParent(this);
        ComponentCreator linkCreator = options.getLinkCreator();
        link = (linkCreator != null) ? ((CoreLink) (linkCreator.create(linkOptions))) : getComponentFactory().createLink(linkOptions);
        getParentComponentPart().add(link);
        return link;
    }

    public CoreBadge getBadge() {
        return badge;
    }
    ...
    private boolean needsBadge(O options) {
        ...
        return options.hasBadgeOptions();
    }

    private CoreBadge createBadge(O options) {
        if ((badge != null) || (options == null))
            return badge;

        CoreBadgeOptions badgeOptions = options.getBadgeOptions();
        if (badgeOptions == null)
            return badge;

        CoreParentComponentPart parent = (link != null) ? link : getParentComponentPart();
        badgeOptions.setParent(link != null ? link : this);
        ComponentCreator badgeCreator = options.getBadgeCreator();
        badge = (badgeCreator != null) ? ((CoreBadge) (badgeCreator.create(badgeOptions))) : getComponentFactory().createBadge(badgeOptions);
        parent.add(badge);
        return badge;
    }
    ...
    private CoreIcon createIcon(O options) {
        if ((icon != null) || (options == null))
            return icon;

        CoreIconOptions iconOptions = options.getIconOptions();
        if (iconOptions == null)
            return icon;

        CoreParentComponentPart parent = (link != null) ? link : getParentComponentPart();
        iconOptions.setParent(link != null ? link : this);
        ComponentCreator iconCreator = options.getIconCreator();
        icon = (iconCreator != null) ? ((CoreIcon) (iconCreator.create(iconOptions))) : getComponentFactory().createIcon(iconOptions);
        parent.add(icon);
        return icon;
    }
    ...
    private CoreInlineText createLabelContainer(O options) {
        ...
        CoreParentComponentPart parent = (link != null) ? link : getParentComponentPart();
        labelContainerOptions.setParent(link != null ? link : this);
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreTabPart component = ((CoreTabPart) (options.getComponent()));
        if (component == null)
        ...
        applicator.add(options.getBadgeVisibleOption(), component::setBadgeVisible);
        applicator.add(options.getBadgeLabelOption(), component::setBadgeLabel);
    }
