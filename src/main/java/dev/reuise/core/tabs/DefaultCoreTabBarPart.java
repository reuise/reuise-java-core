...
public abstract class DefaultCoreTabBarPart<S extends DefaultCoreTabBarPart<S, O>, O extends CoreTabBarPartOptions> implements ComponentPart , CoreTabBar {
    ...
    protected DefaultCoreTabBarPart(O options) {
    }
    ...
    @Override
    public CoreTabBar getComponent() {
        return ((CoreTabBar) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        container = createContainer(options);
        activeIndicator = createActiveIndicator(options);
        setupReferences();
        return true;
    }
    ...
    public CoreContainer getActiveIndicator() {
    ...
    private CoreContainer createActiveIndicator(O options) {
        if ((activeIndicator != null) || (options == null))
            return activeIndicator;

        CoreContainerOptions activeIndicatorOptions = options.getActiveIndicatorOptions();
        if (activeIndicatorOptions == null)
            return activeIndicator;

        CoreParentComponentPart parent = (container != null) ? container : getParentComponentPart();
        activeIndicatorOptions.setParent(container != null ? container : this);
        ComponentCreator activeIndicatorCreator = options.getActiveIndicatorCreator();
        activeIndicator = (activeIndicatorCreator != null) ? ((CoreContainer) (activeIndicatorCreator.create(activeIndicatorOptions))) : getComponentFactory().createContainer(activeIndicatorOptions);
        parent.add(activeIndicator);
        return activeIndicator;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        ...
        applicator.add(options.getTabsOption(), component::setTabs);
        applicator.add(options.getActiveTabOption(), component::setActiveTab);
    }
    ...
    // Implementation
    public S setActiveTab(CoreTab tab) {
    ...
    // Implementation
    public S setActiveTab(String text) {
        CoreTab tab = getTabByLabel(text);
        if (tab != null)
