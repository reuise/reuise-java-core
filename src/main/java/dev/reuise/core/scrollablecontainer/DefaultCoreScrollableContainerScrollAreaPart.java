...
public abstract class DefaultCoreScrollableContainerScrollAreaPart<S extends DefaultCoreScrollableContainerScrollAreaPart<S, O>, O extends CoreScrollableContainerScrollAreaPartOptions> implements ComponentPart , CoreScrollableContainerScrollArea {
    private CoreParentComponentPart parentComponentPart;

    private CoreBaseComponentPart baseComponentPart;

    protected CoreContainer scrollAreaTopFade;

    protected CoreContainer scrollAreaLeftFade;

    protected CoreContainer scrollAreaContent;

    protected CoreContainer scrollAreaBottomFade;

    protected CoreContainer scrollAreaRightFade;

    protected ScrollableContainerFadeOptions fadeOptions;

    protected Integer borderRadius;

    protected DefaultCoreScrollableContainerScrollAreaPart(O options) {
    }
    ...
    @Override
    public CoreScrollableContainerScrollArea getComponent() {
        return ((CoreScrollableContainerScrollArea) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        if (needsScrollAreaTopFade(options)) {
            scrollAreaTopFade = createScrollAreaTopFade(options);
        }
        if (needsScrollAreaLeftFade(options)) {
            scrollAreaLeftFade = createScrollAreaLeftFade(options);
        }
        scrollAreaContent = createScrollAreaContent(options);
        if (needsScrollAreaBottomFade(options)) {
            scrollAreaBottomFade = createScrollAreaBottomFade(options);
        }
        if (needsScrollAreaRightFade(options)) {
            scrollAreaRightFade = createScrollAreaRightFade(options);
        }
        setupReferences();
        return true;
    }

    public CoreContainer getScrollAreaTopFade() {
        return scrollAreaTopFade;
    }
    ...
    private boolean needsScrollAreaTopFade(O options) {
        ...
        if ((options.getScrollDirection() == ScrollDirection.VERTICAL) || (options.getScrollDirection() == ScrollDirection.BOTH))
            return true;

        return false;
    }

    private CoreContainer createScrollAreaTopFade(O options) {
        if ((scrollAreaTopFade != null) || (options == null))
            return scrollAreaTopFade;

        CoreContainerOptions scrollAreaTopFadeOptions = options.getScrollAreaTopFadeOptions();
        if (scrollAreaTopFadeOptions == null)
            return scrollAreaTopFade;

        scrollAreaTopFadeOptions.setParent(this);
        ComponentCreator scrollAreaTopFadeCreator = options.getScrollAreaTopFadeCreator();
        scrollAreaTopFade = (scrollAreaTopFadeCreator != null) ? ((CoreContainer) (scrollAreaTopFadeCreator.create(scrollAreaTopFadeOptions))) : getComponentFactory().createContainer(scrollAreaTopFadeOptions);
        getParentComponentPart().add(scrollAreaTopFade);
        return scrollAreaTopFade;
    }

    public CoreContainer getScrollAreaLeftFade() {
        return scrollAreaLeftFade;
    }
    ...
    private boolean needsScrollAreaLeftFade(O options) {
        ...
        if ((options.getScrollDirection() == ScrollDirection.HORIZONTAL) || (options.getScrollDirection() == ScrollDirection.BOTH))
            return true;

        return false;
    }

    private CoreContainer createScrollAreaLeftFade(O options) {
        if ((scrollAreaLeftFade != null) || (options == null))
            return scrollAreaLeftFade;

        CoreContainerOptions scrollAreaLeftFadeOptions = options.getScrollAreaLeftFadeOptions();
        if (scrollAreaLeftFadeOptions == null)
            return scrollAreaLeftFade;

        scrollAreaLeftFadeOptions.setParent(this);
        ComponentCreator scrollAreaLeftFadeCreator = options.getScrollAreaLeftFadeCreator();
        scrollAreaLeftFade = (scrollAreaLeftFadeCreator != null) ? ((CoreContainer) (scrollAreaLeftFadeCreator.create(scrollAreaLeftFadeOptions))) : getComponentFactory().createContainer(scrollAreaLeftFadeOptions);
        getParentComponentPart().add(scrollAreaLeftFade);
        return scrollAreaLeftFade;
    }

    public CoreContainer getScrollAreaContent() {
    ...
    private CoreContainer createScrollAreaContent(O options) {
        if ((scrollAreaContent != null) || (options == null))
            return scrollAreaContent;

        CoreContainerOptions scrollAreaContentOptions = options.getScrollAreaContentOptions();
        if (scrollAreaContentOptions == null)
            return scrollAreaContent;

        scrollAreaContentOptions.setParent(this);
        ComponentCreator scrollAreaContentCreator = options.getScrollAreaContentCreator();
        scrollAreaContent = (scrollAreaContentCreator != null) ? ((CoreContainer) (scrollAreaContentCreator.create(scrollAreaContentOptions))) : getComponentFactory().createContainer(scrollAreaContentOptions);
        getParentComponentPart().add(scrollAreaContent);
        return scrollAreaContent;
    }

    @Override
    public S add(CoreComponent child) {
        scrollAreaContent.add(child);
    ...
    public void remove(CoreComponent child) {
        scrollAreaContent.remove(child);
    }

    @Override
    public void removeAll() {
        scrollAreaContent.removeAll();
    }

    @Override
    public List<CoreComponent> getChildren() {
        return scrollAreaContent.getChildren();
    }

    public CoreContainer getScrollAreaBottomFade() {
        return scrollAreaBottomFade;
    }
    ...
    private CoreContainer createScrollAreaBottomFade(O options) {
        if ((scrollAreaBottomFade != null) || (options == null))
            return scrollAreaBottomFade;

        CoreContainerOptions scrollAreaBottomFadeOptions = options.getScrollAreaBottomFadeOptions();
        if (scrollAreaBottomFadeOptions == null)
            return scrollAreaBottomFade;

        scrollAreaBottomFadeOptions.setParent(this);
        ComponentCreator scrollAreaBottomFadeCreator = options.getScrollAreaBottomFadeCreator();
        scrollAreaBottomFade = (scrollAreaBottomFadeCreator != null) ? ((CoreContainer) (scrollAreaBottomFadeCreator.create(scrollAreaBottomFadeOptions))) : getComponentFactory().createContainer(scrollAreaBottomFadeOptions);
        getParentComponentPart().add(scrollAreaBottomFade);
        return scrollAreaBottomFade;
    }

    public CoreContainer getScrollAreaRightFade() {
        return scrollAreaRightFade;
    }
    ...
    private CoreContainer createScrollAreaRightFade(O options) {
        if ((scrollAreaRightFade != null) || (options == null))
            return scrollAreaRightFade;

        CoreContainerOptions scrollAreaRightFadeOptions = options.getScrollAreaRightFadeOptions();
        if (scrollAreaRightFadeOptions == null)
            return scrollAreaRightFade;

        scrollAreaRightFadeOptions.setParent(this);
        ComponentCreator scrollAreaRightFadeCreator = options.getScrollAreaRightFadeCreator();
        scrollAreaRightFade = (scrollAreaRightFadeCreator != null) ? ((CoreContainer) (scrollAreaRightFadeCreator.create(scrollAreaRightFadeOptions))) : getComponentFactory().createContainer(scrollAreaRightFadeOptions);
        getParentComponentPart().add(scrollAreaRightFade);
        return scrollAreaRightFade;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreScrollableContainerScrollAreaPart component = ((CoreScrollableContainerScrollAreaPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getScrollDirectionOption(), component::setScrollDirection);
        applicator.add(options.getFadeOptionsOption(), component::setFadeOptions);
        applicator.add(options.getBorderRadiusOption(), component::setBorderRadius);
        applicator.add(options.getFillColorOption(), component::setFillColor);
    }
