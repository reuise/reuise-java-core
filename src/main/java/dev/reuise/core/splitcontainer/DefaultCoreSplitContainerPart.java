...
public abstract class DefaultCoreSplitContainerPart<S extends DefaultCoreSplitContainerPart<S, O>, O extends CoreSplitContainerPartOptions> implements ComponentPart , CoreSplitContainer {
    ...
    protected CoreSplitContainerPanel startPanel;

    protected CoreSplitContainerDivider divider;

    protected CoreSplitContainerPanel endPanel;

    protected Map<State, SplitDirection> direction = new HashMap<>();

    protected Map<State, Boolean> reverseDirection = new HashMap<>();

    protected Boolean resizable;

    protected Map<State, Object> splitPosition = new HashMap<>();

    protected Map<State, Object> minSplitPosition = new HashMap<>();

    protected Map<State, Object> maxSplitPosition = new HashMap<>();

    protected DefaultCoreSplitContainerPart(O options) {
    }
    ...
    @Override
    public CoreSplitContainer getComponent() {
        return ((CoreSplitContainer) (baseComponentPart.getComponent()));
    }

    public boolean onPreInitialize(O options) {
        // Layout children
        container = createContainer(options);
        startPanel = createStartPanel(options);
        if (needsDivider(options)) {
            divider = createDivider(options);
        }
        endPanel = createEndPanel(options);
        setupReferences();
        return true;
    }
    ...
    public CoreSplitContainerPanel getStartPanel() {
        return startPanel;
    }

    private boolean needsStartPanel(O options) {
        if (startPanel != null)
            return true;
    ...
    private CoreSplitContainerPanel createStartPanel(O options) {
        if ((startPanel != null) || (options == null))
            return startPanel;

        CoreSplitContainerPanelOptions startPanelOptions = options.getStartPanelOptions();
        if (startPanelOptions == null)
            return startPanel;

        CoreParentComponentPart parent = (container != null) ? container : getParentComponentPart();
        startPanelOptions.setParent(container != null ? container : this);
        ComponentCreator startPanelCreator = options.getStartPanelCreator();
        startPanel = (startPanelCreator != null) ? ((CoreSplitContainerPanel) (startPanelCreator.create(startPanelOptions))) : getComponentFactory().createSplitContainerPanel(startPanelOptions);
        parent.add(startPanel);
        return startPanel;
    }

    public CoreSplitContainerDivider getDivider() {
        return divider;
    }

    private boolean needsDivider(O options) {
        if (divider != null)
            return true;
    ...
    private CoreSplitContainerDivider createDivider(O options) {
        if ((divider != null) || (options == null))
            return divider;

        CoreSplitContainerDividerOptions dividerOptions = options.getDividerOptions();
        if (dividerOptions == null)
            return divider;

        CoreParentComponentPart parent = (container != null) ? container : getParentComponentPart();
        dividerOptions.setParent(container != null ? container : this);
        ComponentCreator dividerCreator = options.getDividerCreator();
        divider = (dividerCreator != null) ? ((CoreSplitContainerDivider) (dividerCreator.create(dividerOptions))) : getComponentFactory().createSplitContainerDivider(dividerOptions);
        parent.add(divider);
        return divider;
    }

    public CoreSplitContainerPanel getEndPanel() {
        return endPanel;
    }

    private boolean needsEndPanel(O options) {
        if (endPanel != null)
            return true;
    ...
    private CoreSplitContainerPanel createEndPanel(O options) {
        if ((endPanel != null) || (options == null))
            return endPanel;

        CoreSplitContainerPanelOptions endPanelOptions = options.getEndPanelOptions();
        if (endPanelOptions == null)
            return endPanel;

        CoreParentComponentPart parent = (container != null) ? container : getParentComponentPart();
        endPanelOptions.setParent(container != null ? container : this);
        ComponentCreator endPanelCreator = options.getEndPanelCreator();
        endPanel = (endPanelCreator != null) ? ((CoreSplitContainerPanel) (endPanelCreator.create(endPanelOptions))) : getComponentFactory().createSplitContainerPanel(endPanelOptions);
        parent.add(endPanel);
        return endPanel;
    }
    ...
    public void insertBefore(CoreComponent child, CoreComponent beforeChild) {
        endPanel.insertBefore(child, beforeChild);
    }

    @Override
    public S addText(String text) {
        endPanel.addText(text);
        return self();
    }

    @Override
    public void remove(CoreComponent child) {
        endPanel.remove(child);
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreSplitContainerPart component = ((CoreSplitContainerPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getDirectionOption(), component::setDirection);
        applicator.add(options.getReverseDirectionOption(), component::setReverseDirection);
        applicator.add(options.getResizableOption(), component::setResizable);
        applicator.add(options.getSplitPositionOption(), component::setSplitPosition);
        applicator.add(options.getMinSplitPositionOption(), component::setMinSplitPosition);
        applicator.add(options.getMaxSplitPositionOption(), component::setMaxSplitPosition);
        if (states != null)
            states.stream().forEach((State state) -> {
                // Apply options for each (non-default) state
                applicator.add(options.getDirectionOption(state), state, component::setDirection);
                applicator.add(options.getReverseDirectionOption(state), state, component::setReverseDirection);
                applicator.add(options.getSplitPositionOption(state), state, component::setSplitPosition);
                applicator.add(options.getMinSplitPositionOption(state), state, component::setMinSplitPosition);
                applicator.add(options.getMaxSplitPositionOption(state), state, component::setMaxSplitPosition);
            });
    ...
    public S setDirection(SplitDirection direction, State state) {
        this.direction.put(state, direction);
        return self();
    }

    @Override
    public boolean isReverseDirection() {
        return Boolean.TRUE.equals(reverseDirection.get(null));
    }

    @Override
    public boolean isReverseDirection(State state) {
        return Boolean.TRUE.equals(reverseDirection.get(state));
    }

    @Override
    public S setReverseDirection(Boolean reverseDirection) {
        this.reverseDirection.put(null, reverseDirection);
        return self();
    }
    ...
    public S setReverseDirection(Boolean reverseDirection, State state) {
        this.reverseDirection.put(state, reverseDirection);
        return self();
    }

    @Override
    public boolean isResizable() {
        return Boolean.TRUE.equals(resizable);
    }
    ...
    public Object getSplitPosition() {
        return splitPosition.get(null);
    }

    @Override
    public Object getSplitPosition(State state) {
        return splitPosition.get(state);
    }

    @Override
    public S setSplitPosition(Object splitPosition) {
        this.splitPosition.put(null, splitPosition);
        return self();
    }
    ...
    public S setSplitPosition(Object splitPosition, State state) {
        this.splitPosition.put(state, splitPosition);
        return self();
    }

    @Override
    public Object getMinSplitPosition() {
        return minSplitPosition.get(null);
    }

    @Override
    public Object getMinSplitPosition(State state) {
        return minSplitPosition.get(state);
    }

    @Override
    public S setMinSplitPosition(Object minSplitPosition) {
        this.minSplitPosition.put(null, minSplitPosition);
        return self();
    }
    ...
    public S setMinSplitPosition(Object minSplitPosition, State state) {
        this.minSplitPosition.put(state, minSplitPosition);
        return self();
    }

    @Override
    public Object getMaxSplitPosition() {
        return maxSplitPosition.get(null);
    }

    @Override
    public Object getMaxSplitPosition(State state) {
        return maxSplitPosition.get(state);
    }

    @Override
    public S setMaxSplitPosition(Object maxSplitPosition) {
        this.maxSplitPosition.put(null, maxSplitPosition);
        return self();
    }
    ...
    public S setMaxSplitPosition(Object maxSplitPosition, State state) {
        this.maxSplitPosition.put(state, maxSplitPosition);
        return self();
    }
