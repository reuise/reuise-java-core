...
public abstract class DefaultCoreSplitContainerDividerPart<S extends DefaultCoreSplitContainerDividerPart<S, O>, O extends CoreSplitContainerDividerPartOptions> implements ComponentPart , CoreSplitContainerDivider {
    ...
    protected Map<State, Integer> thickness = new HashMap<>();

    protected DefaultCoreSplitContainerDividerPart(O options) {
    }
    ...
    @Override
    public CoreSplitContainerDivider getComponent() {
        return ((CoreSplitContainerDivider) (baseComponentPart.getComponent()));
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreSplitContainerDividerPart component = ((CoreSplitContainerDividerPart) (options.getComponent()));
        if (component == null)
        ...
        if (states != null)
            states.stream().forEach((State state) -> {
                ...
                applicator.add(options.getThicknessOption(state), state, component::setThickness);
            });
    ...
    public Integer getThickness() {
        return thickness.get(null);
    }

    @Override
    public Integer getThickness(State state) {
        return thickness.get(state);
    }

    @Override
    public S setThickness(Integer thickness) {
        this.thickness.put(null, thickness);
        return self();
    }
    ...
    public S setThickness(Integer thickness, State state) {
        this.thickness.put(state, thickness);
        return self();
    }
