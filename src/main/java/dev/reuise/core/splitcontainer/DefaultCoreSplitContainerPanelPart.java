...
public abstract class DefaultCoreSplitContainerPanelPart<S extends DefaultCoreSplitContainerPanelPart<S, O>, O extends CoreSplitContainerPanelPartOptions> implements ComponentPart , CoreSplitContainerPanel {
    ...
    protected Map<State, Object> size = new HashMap<>();

    protected Map<State, Object> minSize = new HashMap<>();

    protected Map<State, Object> maxSize = new HashMap<>();

    protected Map<State, SplitDirection> direction = new HashMap<>();

    protected DefaultCoreSplitContainerPanelPart(O options) {
    }
    ...
    @Override
    public CoreSplitContainerPanel getComponent() {
        return ((CoreSplitContainerPanel) (baseComponentPart.getComponent()));
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        CoreSplitContainerPanelPart component = ((CoreSplitContainerPanelPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getCollapsedOption(), component::setCollapsed);
        ...
        if (states != null)
            states.stream().forEach((State state) -> {
                ...
                applicator.add(options.getCollapsedOption(state), state, component::setCollapsed);
                applicator.add(options.getSizeOption(state), state, component::setSize);
                applicator.add(options.getMinSizeOption(state), state, component::setMinSize);
                applicator.add(options.getMaxSizeOption(state), state, component::setMaxSize);
    ...
    public S setSize(Object size, State state) {
        this.size.put(state, size);
        return self();
    }

    @Override
    public Object getMinSize() {
        return minSize.get(null);
    }

    @Override
    public Object getMinSize(State state) {
        return minSize.get(state);
    }

    @Override
    public S setMinSize(Object minSize) {
        this.minSize.put(null, minSize);
        return self();
    }
    ...
    public S setMinSize(Object minSize, State state) {
        this.minSize.put(state, minSize);
        return self();
    }

    @Override
    public Object getMaxSize() {
        return maxSize.get(null);
    }

    @Override
    public Object getMaxSize(State state) {
        return maxSize.get(state);
    }

    @Override
    public S setMaxSize(Object maxSize) {
        this.maxSize.put(null, maxSize);
        return self();
    }
    ...
    public S setMaxSize(Object maxSize, State state) {
        this.maxSize.put(state, maxSize);
        return self();
    }
